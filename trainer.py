...
class Trainer(object):
    ...
    def train(self, n_epochs: int = 1000, n_steps=10, save=True):
        ...
        tbar = tqdm(range(n_steps), desc='Training', position=0, leave=True)
        for epoch_idx in range(n_epochs):
            for step_idx in tbar:
                ...
                # Update progress bar with epoch, error and loss information
                tbar.set_postfix({
                    'Epoch': f'{epoch_idx}/{n_epochs}',
                    'Error': f'{int(100*err)}cm',
                    'Loss': f'{loss:.2f}'
                })

            if save:
                # Show save status in progress bar
                tbar.set_postfix_str('Saving checkpoint...')
