...
def generate_run_ID(options):
    ...
    params = [
        ...
        'sequence', str(options.sequence_length),
...
def load_trained_weights(model, trainer, weight_dir):
    ...
    state_dict = {}
    state_dict['encoder.weight'] = torch.from_numpy(weights[0]).float().T
    state_dict['RNN.weight_ih_l0'] = torch.from_numpy(weights[1]).float().T
    state_dict['RNN.weight_hh_l0'] = torch.from_numpy(weights[2]).float().T
    state_dict['decoder.weight'] = torch.from_numpy(weights[3]).float().T
